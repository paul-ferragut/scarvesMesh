...
void ofxSimpleSlider::updateLayout(ofRectangle rect) {
	x = rect.x;
	y = rect.y;
	width = rect.width;
	height = rect.height;
...
//----------------------------------------------------
void ofxSimpleSlider::mouseMoved(int xm, int ym) {
	bHasFocus = false;
}
void ofxSimpleSlider::mouseDragged(int xm, int ym) {
	if (bHasFocus) {
		updatePercentFromMouse(xm,ym);
	}
}
void ofxSimpleSlider::mousePressed(int xm,int ym) {
	bHasFocus = false;
	if (box.inside(xm, ym)) {
		bHasFocus = true;
		updatePercentFromMouse(xm, ym);
...
void ofxSimpleSlider::mouseReleased(int xm, int ym) {
	if (bHasFocus) {
		if (box.inside(xm, ym)) {
			updatePercentFromMouse(xm, ym);
		}
